...
To start using a permissions mod with Essential Commands, set `use_permissions_api` to `true` in the file `config/EssentialCommands.properties`, install your chosen permission mod, and restart the server.

Choose one of the Tabs below depending on which permissions mod you chose to use:
...
Player roles is primarily managed with a single config file, `config/roles.json`.

A simple starter configuration for Essential commands might look something like this:
...
This creates a role `admin`, with the ability to create and delete `warp`s, and grants all players the ability to use the `tpa` and `home` features, and the ability to teleport to `warp`s.
